...
namespace KSPCurveBuilder
{
    public class CurveViewController
    {
        ...
        private float CalculateOptimalZoom(float minTime, float maxTime, float minValue, float maxValue)
        {
            ...
            // Zoom out if curve extends beyond keys
        ...
        {
            var points = _editorService.Points;
            if (points.Count == 0) return new PointF(0.5f, 0.5f);
            ...
            // Center vertically on curve midpoint
            float panY = 0.5f + (curveMid - keyMid) / (keyMax - keyMin) * 0.5f;
            return new PointF(0.5f, Math.Max(0.01f, Math.Min(0.99f, panY)));
        }
        ...
        private void OnMouseDown(object? sender, MouseEventArgs e)
        {
            if (e?.Button == MouseButtons.Left)
            {
                ...
                if (hitIndex >= 0 && !_isPanning)
                {
                    ...
                    // Store reference to original point
        ...
        private void OnMouseMove(object? sender, MouseEventArgs e)
        {
            if (_isDraggingPoint && _draggedPointIndex >= 0)
            {
                ...
                {
                    var points = _editorService.PointsInternal; // Direct access
        ...
        private void OnMouseUp(object? sender, MouseEventArgs e)
        {
            if (_isDraggingPoint)
            {
                ...
                // CRITICAL: Only now fire the drag ended event for undo/textbox update
        ...
        private void OnMouseClick(object? sender, MouseEventArgs e)
        {
            if (e?.Button == MouseButtons.Right)
            {
                ...
                // RIGHT-click adds a point - fire drag ended to update textbox
                DragEnded?.Invoke(this, EventArgs.Empty);
            }
        }
        ...
        private (float minTime, float maxTime, float minValue, float maxValue) CalculateDataRange()
        {
            ...
            // Sample curve to find actual vertical extents
            var curve = _getCurve();
    ...
    public class PointDragEventArgs : EventArgs
    {
        public int Index { get; }
        public FloatString4 NewPoint { get; }

        public PointDragEventArgs(int index, FloatString4 newPoint)
        {
            Index = index;
            NewPoint = newPoint;
        }
