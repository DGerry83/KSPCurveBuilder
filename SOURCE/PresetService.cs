...
namespace KSPCurveBuilder
{
    ...
    public class PresetService
    {
        ...
        {
            var builtIns = BuiltInPresets.GetAll() ?? new List<Preset>();
            var userNames = PresetManager.GetAvailablePresets() ?? Array.Empty<string>();
            var userPresets = userNames.Select(name => PresetManager.LoadPreset(name))
                                      .Where(p => p != null)
                                      .ToArray();
        ...
        {
            name = System.IO.Path.GetInvalidFileNameChars().Aggregate(name, (current, c) => current.Replace(c, '_'));
