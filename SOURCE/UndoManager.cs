...
namespace KSPCurveBuilder
{
    ...
    /// </summary>
    public sealed class SimpleUndoManager
    {
        ...
        public SimpleUndoManager(string initialState)
        {
            _currentState = initialState;
        }

        /// <summary>
        /// Records a new action, saving the current text state to undo stack.
        /// Automatically clears redo stack.
        /// </summary>
        public void RecordAction(string newState, string actionName)
        {
            _undoStack.Push(new UndoState(_currentState, actionName));
        ...
        /// <summary>
        /// Undoes the last action and returns the previous text state.
        /// </summary>
        public string Undo()
        {
            if (!CanUndo) return _currentState;
            _redoStack.Push(new UndoState(_currentState, "Redo"));
        ...
        /// <summary>
        /// Redoes the previously undone action and returns the next text state.
        /// </summary>
        public string Redo()
        {
            if (!CanRedo) return _currentState;
            _undoStack.Push(new UndoState(_currentState, "Undo"));
